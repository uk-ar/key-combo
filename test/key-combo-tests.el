...
(defun kc-test-execute-command (cmd &optional buffer no-erase)
...
;; (defun kc-test (cmd)
;;   (kc-test-execute-command cmd (current-buffer) t)
;;   nil)
...
(ert-deftest test-kc-test-execute-commands ()
  (should (string= (kc-test-execute-command "a") "a"))
  (let ((last-command-event ?b))
    (should (string= (kc-test-execute-command 'self-insert-command) "b")))
  (should (string= (kc-test-execute-command (lambda () (insert "a"))) "a"))
  (with-temp-buffer
    (switch-to-buffer (current-buffer))
    (key-combo-command-execute (kc-get-command "a`!!'a"))
    (should (string= (buffer-string) "aa"))
    (should (eq (point) 2)))
  (should-error (kc-test-execute-command 'wrong-command)))
...
(ert-deftest test-kc-special ()
  (with-temp-buffer`
   (let ((bf (current-buffer)))
     ...
     (insert "\"")
     (should (string= (kc-test-execute-command "=" bf t) "\"="))
     (erase-buffer)
     (insert ";")
     (should (string= (kc-test-execute-command "=" bf t) ";="))
     (erase-buffer)
     ...
     (insert "\"\"a")
     (goto-char 3)
     (should (string= (kc-test-execute-command "." bf t) "\"\".a"))
     (erase-buffer)
     (insert "\"\"")
     (goto-char 3)
     (should (string= (kc-test-execute-command "." bf t) "\"\"."))
...
(ert-deftest test-kc-vectors ()
  (with-temp-buffer`
   (let ((bf (current-buffer)))
     (kc-test-minor-mode 1)
     (buffer-enable-undo)
     (should (string= (kc-test-execute-command "=") " = "))
     (should (string= (kc-test-execute-command "==") " == "))
     (should (string= (kc-test-execute-command [?=]) " = "))
     (should (string= (kc-test-execute-command [?= ?=]) " == "))
     (should (string= (kc-test-execute-command [?= ?>]) " => "))
...
(ert-deftest test-kc-undo ()
	...
	(key-combo-define-kalist kc-mode-map key-combo-common-default)
	(insert "init:")
	(should (string= (kc-test-execute-command "=" bf t) "init:= "))
	(undo -1)
	(should (string= (buffer-string) "init:="))
	(undo-more 1)
	(should (string= (buffer-string) ""))
	(should (string= (kc-test-execute-command "=" bf) " = "))
	(undo)
	(should (string= (buffer-string) "="))
	(should (string= (kc-test-execute-command "= C-x u" bf) "="))
	(should (string= (kc-test-execute-command "== C-x u" bf) " = ")))))
...
(ert-deftest test-kc-lisp-mode ()
  (with-temp-buffer
    ...
    (let ((bf (current-buffer)))
      (should (string= (kc-test-execute-command ";." bf) ";; ."))
      (should (string= (kc-test-execute-command "=" bf) "= "))
      (should (string= (kc-test-execute-command "==" bf) "eq "))
      (should (string= (kc-test-execute-command "," bf) ","))
      (should (string= (kc-test-execute-command ",," bf) ",,"))
      (should (string= (kc-test-execute-command "." bf) "."))
      (should (string= (kc-test-execute-command ". SPC" bf) " . "))
      (should (string= (kc-test-execute-command ";" bf) ";; "))
      (should (string= (kc-test-execute-command ";," bf) ";; ,"))
      (erase-buffer)
      (insert ";")
